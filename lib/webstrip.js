 ...
 */
export async function webstrip(url, options) {
 ...
 * @returns - A promise that resolves to the webstrip result.
 */
async function webstripReq(url, reqOptions) {
    const redirectCount = 0;
    return _webstripReq(url, reqOptions, redirectCount);
}
async function _webstripReq(url, reqOptions, redirectCount = 0) {
...
async function waitForTargetDestroyed(browser, cb) {
 ...
 * from the URL.
 */
async function webstripNav(url, options) {
    const nav = getNavInfo(options?.navigate);
    ...
    await page.setExtraHTTPHeaders(buildReqHeaders(options?.headerOptions));
    let redirectCount = 0;
    let response;
...
//# sourceMappingURL=webstrip.js.map