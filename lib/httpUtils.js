 ...
 */
export function getRandomAcceptEncoding() {
    return randomItem(acceptEncodings);
}
/**
 * Returns a random "Accept" (MIME) header string for emulating a browser.
 */
export function getRandomAccept() {
    return randomItem(acceptMimes);
}
function getHeaderValue(list, inclusion, anyValue) {
    if (!inclusion || inclusion === 'default')
        return list[0];
    if (inclusion === 'any' && anyValue)
        return anyValue;
    if (inclusion === 'random')
 ...
 */
export function getReqHeaders(opts = {}) {
    let headers = {
 ...
 */
export function buildReqHeaders(reqOptions) {
    return {
